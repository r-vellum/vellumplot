...
test_that("guides(color = 'none') drops the colour legend", {
  p <- vplot(df) |> mark_point(x = wt, y = mpg, color = cyl) |> guides(color = "none")
  expect_length(quill:::.legend_guides(quill:::.build_panels(p)$scales), 0L)
})

test_that("guide_none() hides a legend even when no scale is declared", {
  p <- vplot(df) |> mark_point(x = wt, y = mpg, color = cyl) |> guides(color = guide_none())
  expect_length(quill:::.legend_guides(quill:::.build_panels(p)$scales), 0L)
...
test_that("guide_legend(reverse=) reverses the key order only", {
  ...
  base <- quill:::.build_panels(vplot(df) |> mark_point(x = wt, y = mpg, color = cyl))
...
test_that("guide_legend(title=) overrides the legend title", {
  ...
  expect_identical(quill:::.build_panels(p)$scales$color$name, "Cylinders")
})
...
test_that("guides() applies to size/shape/alpha/linetype and requires named args", {
  p <- vplot(df) |>
    ...
    guides(size = "none")
  expect_length(quill:::.legend_guides(quill:::.build_panels(p)$scales), 0L)
  expect_error(vplot(df) |> mark_point(x = wt, y = mpg) |> guides("none"), "named")
})
