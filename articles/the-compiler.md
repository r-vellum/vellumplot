...
**One scene, many outputs.** The compiler targets a `vellum` scene, not
a device. The same spec renders to a raster or a vector file, and the
same scene model is what the `vellumwidget` package turns into an
interactive widget. Write the plot once; choose the output later.
